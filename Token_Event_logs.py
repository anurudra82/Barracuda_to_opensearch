...
WAAS_API_TOKEN = "eyJhY2NfaWQiOiAxMTIyNzM0NywgInVzZXJfaWQiOiAyMDg1ODIzNTIsICJleHBpcmF0aW9uIjogMTc0ODA5MzU2NX0=.3dbf4c71872c06e0cd86da8e828b9445c4c"  # <-- Place your Barracuda WAAS API token here
...
def fetch_waas_logs():
    """Fetch logs from Barracuda WAAS using only the API token."""
    ...
    logs = logs_response.get('results', [])
...
def main():
    if not WAAS_API_TOKEN:
        print("[!] No API token provided. Please set WAAS_API_TOKEN.")
        return

    logs = fetch_waas_logs()
    if not logs:
        print("[!] No logs fetched. This may be due to insufficient token permissions or API limitations.")
